...
export const makeEmail = (template, bodyElements) => {
  ...
  const logo = makeElement(template, 'logo', {
    url:
      'https://firebasestorage.googleapis.com/v0/b/production2hats.appspot.com/o/assets%2FemailTempate2hatLogo.png?alt=media&token=aba63f32-8c84-4dbe-926a-58f9223ee46f',
  });
  ...
  personalisables.forEach(personalisable => {
    for (const field in personalisable) {
      for (const replaceable in element.replaceables) {
        output.replaceables[replaceable] = globalReplace(
          output.replaceables[replaceable],
          `#${field}#`,
          personalisable[field]
        );
      }
    }
  });
