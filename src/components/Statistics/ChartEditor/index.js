...
const getChartToEdit = chartToEdit => {
  ...
  if (chartToEdit.trackers) {
    trackers = chartToEdit.trackers.map(x => ({
      id: x.id,
      colour: x.colour,
      preset: x.presetName,
    }));
  ...
  if (chartToEdit.queries) {
    queries = chartToEdit.queries.map(x => ({
      id: x.id,
      colour: x.colour,
      preset: x.presetName,
    }));
...
function ChartEditor(props) {
  ...
  if (chart.type === 'number') {
    ...
    if (!lastItem.preset || !lastItem.colour) disableAddItemButton = true;
  }
  ...
  const handleChangeChartType = (e, val) => {
    ...
    if (chart.items.length > 0) {
      switch (val) {
        case 'percentage':
          newItems = chart.items.slice(0, 2);
          break;
        case 'number':
          newItems = chart.items.slice(0, 1);
          break;
        default:
          newItems = chart.items;
          break;
      }
    } else {
  ...
  const handleDone = () => {
    ...
    const transformedItems = output.items.map(x => ({
      ...trackers[x.preset],
      colour: x.colour,
      id: x.id,
    }));
  ...
  return (
    ...
    >
      <DialogTitle className={classes.dialogTitle}>
        {chartToEdit
          ? `Edit ${chartToEdit.title ? chartToEdit.title : 'chart'}`
          : 'New chart'}
      </DialogTitle>
          ...
          <>
            {chart.type !== 'number' && (
              <TextField
                value={chart.title}
                onChange={e => {
                  setChart({ ...chart, title: e.target.value });
                }}
                label="Title"
                fullWidth
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={
            ...
            (chart.items.length === 1 && !chart.items[0].preset)
          }
        >
          Done
        </Button>
