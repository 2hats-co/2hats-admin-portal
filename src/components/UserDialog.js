...
function UserDialog(props) {
    ...
    const updateTheme = () => {
        updateProperties(COLLECTIONS.admins, user.UID, { adminPortal: {
            themeColor,
            ...
            <Paper elevation={24} classes={{ root: classes.paperRoot }}>
                <Grid container direction="column" justify="center">
                    ...
                    <Grid item className={classes.header}>
                        <Avatar src={user.avatarURL} className={classes.avatar}>{initials ? initials : null}</Avatar>
                        <Typography variant="display1" className={classes.greeting}>{greeting}, {user.givenName}!</Typography>
                    ...
                    <Grid item className={classes.borderedSection}>
                        <Grid container alignItems="center" justify="center">
                            ...
                            <IconButton onClick={() => { setShowColorPicker(!showColorPicker) }}>
                                <div className={ classes.swatch } style={{background:themeColor}} />
                            </IconButton>
                            ...
                            { showColorPicker ?<Fade in>
                                <div className={ classes.popover }>
                                    <div className={ classes.cover } onClick={() => { setShowColorPicker(!showColorPicker) }}/>
                                    <ChromePicker color={themeColor} onChange={(val) => { setThemeColor(`hsl(${Math.floor(val.hsl.h)}, ${Math.floor(val.hsl.s * 100)}%, ${Math.floor(val.hsl.l * 100)}%)`) }} />
                                </div></Fade>
                            : null }
                    ...
                    <Grid item className={classes.themeButtons}>
                        ...
                        <Button
                            onClick={() => { setThemeColor(ORANGE_COLOR); setDarkTheme(false) }}
                            color="primary"
                        >
                            Reset
                        </Button>
