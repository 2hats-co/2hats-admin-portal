...
function ScreeningForm(props) {
  ...
  const [showDisqualify, setShowDisqualify] = useState(
    submission.outcome === 'disqualified'
  );
  ...
  const updateSubmission = () => {
    ...
    callable(CLOUD_FUNCTIONS.tracker, {
      type: 'talentTeam',
  ...
  if (showDisqualify)
    return (
      <Grid container className={classes.root} direction="column">
        ...
        <Button
          variant={disqualifyType === 'demographic' ? 'contained' : 'outlined'}
          onClick={() => {
            setTemplate(null);
            setEmailReady(false);
            setTimeout(() => {
              setTemplate(outsideDemographic);
            }, 100);
            setDisqualifyType('demographic');
            setShowSend(true);
          }}
          className={classes.disqualifyButton}
          color="primary"
        >
          Demographic Mismatch
        </Button>
        <Button
          variant={disqualifyType === 'industry' ? 'contained' : 'outlined'}
          onClick={() => {
            setTemplate(null);
            setEmailReady(false);
            setTimeout(() => {
              setTemplate(outsideIndusty);
            }, 100);
        ...
        <Button
          variant={disqualifyType === 'invalid' ? 'contained' : 'outlined'}
          onClick={() => {
            setTemplate(null);
            setEmailReady(false);
            const smartKey = generateSmartKey(
              submission.UID,
              `/uploadResume?step=3`
            );
            setSmartLink(smartKey);
            updateUserDocs(submission.UID, [
              {
                collection: COLLECTIONS.profiles,
                updates: { hasSubmit: false },
              },
              {
                collection: COLLECTIONS.users,
                updates: { stage: 'pre-review', status: 'updating' },
              },
            ]);
            setTimeout(() => {
              setTemplate(invalidSubmission);
            }, 100);
      ...
      <Tooltip
        title={
          <>
            ...
            <br />
            {submission.displayName} will <b>not</b> see the reasons you gave
            here.
            <br />
