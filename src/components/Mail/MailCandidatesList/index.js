...
class MailCandidatesList extends Component {
  ...
  getCandidates(filter) {
    ...
    switch (filter) {
      case 'all':
        filters =
          'stage:resume OR stage:pre-review AND status:accepted OR status:rejected OR status:in-review';
        break;
      case 'accepted':
        filters = 'stage:resume AND status:accepted';
        break;
      case 'rejected':
        filters = 'stage:resume AND status:rejected';
        break;
      case 'in-review':
        ...
        <Grid item xs style={{ overflowY: 'scroll' }}>
          <List disablePadding>
            {candidates.map((x, i) => (
              <CandidateItem
                onClick={() => {
                  this.props.setCandidate(x.objectID);
                }}
                data={x}
