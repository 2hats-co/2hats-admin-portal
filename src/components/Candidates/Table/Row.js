  ...
  return (
    <div
      dangerouslySetInnerHTML={{
        __html: s.replace('<em>', '<b>').replace('</em>', '</b>'),
      }}
    />
  );
}
...
function CandidateRow(props) {
  ...
  const email = candidateData.email && {
    ...
    isHighlighted:
      _highlightResult.email.matchedWords &&
      _highlightResult.email.matchedWords.length !== 0,
  };
  const phoneNumber = candidateData.phoneNumber && {
    value: candidateData.phoneNumber,
    isHighlighted:
      _highlightResult.phoneNumber.matchedWords &&
      _highlightResult.phoneNumber.matchedWords.length !== 0,
  };
  ...
  return (
    <TableRow
      hover
      onClick={() => {
        changeHandler('selectedCandidate', candidateData);
      }}
    >
      ...
      <TableCell
        padding={'none'}
        style={{ maxWidth: 170, overflow: 'hidden', textOverflow: 'ellipsis' }}
      >
        {name}
      </TableCell>
      <TableCell padding={'none'} style={{ maxWidth: 100 }}>
        ...
        {phoneNumber && (
          <ClipBoardIcon
            ...
            isHighlighted={phoneNumber.isHighlighted}
          >
            <PhoneIcon />
          </ClipBoardIcon>
        )}
      ...
      <TableCell padding={'none'}>
        {industry && _bolding(industry.value)}
      </TableCell>
